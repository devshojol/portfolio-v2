    ...
    <div className="relative border-y border-line bg-void/60 py-4">
      <div className="mask-fade-x flex overflow-hidden">
        <div className="animate-marquee flex shrink-0 items-center gap-10 pr-10">
          {row.map((word, i) => (
            <span key={word + i} className="flex shrink-0 items-center gap-10">
              <span className="font-mono text-[13px] uppercase tracking-[0.2em] text-ink-faint transition-colors duration-300 hover:text-accent">
                {word}
              </span>
        ...
        <div aria-hidden className="animate-marquee flex shrink-0 items-center gap-10 pr-10">
          {row.map((word, i) => (
            <span key={`b-${word}-${i}`} className="flex shrink-0 items-center gap-10">
